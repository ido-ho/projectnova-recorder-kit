...
- **`self-test`** (new in 0.5.0) — an editor check you start from the web. It **records nothing and
  ...
  (`Editor/Cloud/SelfTestFacts.cs:31-106`). It
...
|---|---|---|
| Asset list | for every asset under `Assets/`: guid, path, Unity type, file size, and whether it is in the build | phase 1 `:219`, phase 4 `:658` |
...
| **Lines of your C#** | for `.cs` files under `Assets/` up to 1 MiB: every line matching one of SEVEN fixed patterns — `#if UNITY_EDITOR/DEVELOPMENT_BUILD/DEBUG`, a class named like `*Debug*/*Cheat*/*DevMenu*/*DevTool*/*GMTool*/*Console*`, `Random.InitState(`, `RemoteConfig/RemoteSettings/FirebaseRemoteConfig`, `ftue/tutorial/onboarding…=`, `feature_unlock`, `PlayerPrefs.Get/Set…("` — sent as file path, line number and **the matching line's text, trimmed to 200 characters**. No other source is read out. The table is `ExportFormat.cs:357-366` | phase 6 `ExportCollector.cs:971` |
| Docs | `.md`/`.txt` under a project-root `docs/`, `Docs/` or `Documentation/` folder, and `README*` at the project root — up to 1 MiB each | phase 7 `ExportCollector.cs:1056` |
| **Remote config** | any file named `remote_config*.json` anywhere under the project root (not `Library/`, `Temp/`, `Logs/`, `obj/`, `Packages/`, `node_modules/` or dot-folders) — up to 1 MiB each, 200 docs+config files in total. **If yours holds keys or secrets, they are uploaded.** The kit cannot tell and does not try. Check before you press the button | phase 7 `ExportCollector.cs:1056` |
| Art | the ORIGINAL files of: app icons, fonts, audio clips and textures, each up to **8 MiB**, up to **1 GiB** in total; plus a **256-px thumbnail of every texture** (rendered through a temporary RenderTexture — your import settings are not touched). Anything over a cap is not sent and is LISTED by path and size instead | phase 8 `ExportCollector.cs:1127`, thumbnails `:1286` |
...
- Sends anything to the box beyond the job's own result and a status report: a clip and its
  ...
  editor threw). None of it is gathered on purpose. The EXPORT's error strings are scrubbed of
