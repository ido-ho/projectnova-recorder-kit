    ...
    internal static class RelayBoot
    {
        ...
        private static void KeepPlayingWhileUnfocused()
        {
            ...
            EditorApplication.playModeStateChanged += state =>
            {
                ...
                if (!Application.runInBackground)
                    Application.runInBackground = true;
                SweepLeftoverCameraHold();
            };
        }

        /// <summary>
        /// Play-Mode entry's sweep. Leftover camera-hold.json from an aborted run: follow never
        /// writes rotation, so the next session would restore the stale snapshot. Armed holds are
        /// left alone (stills stay posed).
        ///
        /// Under THE PROJECT'S root (<see cref="KitProject.Root"/>), never the working directory
        /// (third audit, M5): the default adapter's bridge writes the snapshot there, and a sweep
        /// that looked anywhere else left it on disk. A method of its own so an EditMode test can
        /// run it — the Play-Mode hook that calls it is registered only outside batch mode.
        /// </summary>
        internal static void SweepLeftoverCameraHold()
        {
            try
            {
                var sweep = CameraPose.SweepOrphaned(CameraPose.Host.ForProject(KitProject.Root()));
                if (!sweep.Ok)
