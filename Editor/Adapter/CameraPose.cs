...
namespace ProjectNova.RecorderKit
{
    ...
    public static class CameraPose
    {
        ...
        /// <summary>
        /// Why a pose that names its own Type is refused on a delivered project (fourth audit, M2):
        /// the ticked camera-spec approved calling its methods on ONE view type, and a command's own
        /// Type used to win over it silently.
        /// </summary>
        public static string TypeIsNotTheSpecsViewType(string type, string viewType) =>
            $"camera-pose names the type '{type}', but the ticked camera-spec approves posing '{viewType}' — " +
            $"refused: on a delivered project a pose moves only the view type its camera-spec names " +
            $"(write '{viewType}', or leave the type out)";
        ...
        public static Outcome Pose(string[] args, Host host)
        {
            ...
            var spec = cam.Value;
            // M9 (second audit) — THE CAMERA BLOCK IS CLOUD CONTENT AND IT NAMES METHODS. The three
            // `set*` names and `viewType` in adapter.json decide which Component this verb finds and
            // which methods it invokes on it; a tick on `camera-pose 1 2 3 …` approved a FRAMING and
            // said nothing about the methods that framing calls. On a gated project the spec is
            // therefore refused until `camera-spec <viewType> <setPosition> <setRotation> <setFov>`
            // is ticked — refused, never quietly run against the kit's defaults instead, because a
            // shot framed by the wrong camera is a wrong recording that nothing downstream catches.
            if (Levers.CameraSpecNeeded(spec))
            {
                var cameraSpecLever = Levers.CameraSpecLever(spec);
                if (!Levers.LeverAllowed(host.ProjectRoot, cameraSpecLever))
                    return Outcome.Fail(Levers.NotApprovedLog(cameraSpecLever));
                // Fourth audit, M2 — THE TICKED SPEC NAMES ONE VIEW TYPE, AND A COMMAND'S OWN TYPE MAY
                // NOT OVERRIDE IT. `camera-pose SaveManager 1 2 3 4 5 6 60` used to look up SaveManager
                // and invoke the approved method names on it, under a tick that named BoardRig. On a
                // delivered project a Type that is not the spec's view type is REFUSED, by name — never
                // silently used, never silently dropped for the spec's own. A spec that names no view
                // type (`camera-spec * …`) leaves the command's Type standing: that command was ticked
                // as written. Locally authored files keep the override (the gate is not live).
                var viewType = (spec.ViewType ?? "").Trim();
                if (parsed.TypeName != null && viewType.Length > 0 && Levers.GateActive(host.ProjectRoot)
                    && !string.Equals(parsed.TypeName, viewType, StringComparison.Ordinal))
                    return Outcome.Fail(TypeIsNotTheSpecsViewType(parsed.TypeName, viewType));
            }
