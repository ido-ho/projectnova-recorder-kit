...
namespace ProjectNova.RecorderKit.Tests
{
    ...
    public class SelfTestStuckTests
    {
        ...
        [Test]
        public void APackagesOwnError_IsNotTheGamesError_ButAGameTriggeredOneIs_AndRichTextIsStripped()
        {
            PlayConsoleWatch.Reset();
            // RL's real 0.9.1 'first error': the Unity MCP plugin, every frame in the PackageCache
            PlayConsoleWatch.OnLog("<color=#ff6b6b>fail:</color> <b>McpManagerClientHub</b> Server forcefully disconnected this plugin. Reason: Authorization failed.",
                "UnityEngine.Debug:LogError (object)\ncom.IvanMurzak.Unity.MCP.Utils.UnityLogger:Log (at ./Library/PackageCache/com.ivanmurzak.unity.mcp@c2a052500f7b/Runtime/Logger/UnityLogger.cs:119)", LogType.Error);
            Assert.AreEqual(0, PlayConsoleWatch.Errors);
            // a package error the GAME triggered: a game frame is in the trace
            PlayConsoleWatch.OnLog("<color=red>Firebase</color> login failed",
                "Firebase.Auth:Sign (at ./Library/PackageCache/com.google.firebase/Auth.cs:10)\nLoginManager:Login (at Assets/Scripts/FirebaseSystem/LoginManager.cs:40)", LogType.Error);
            Assert.AreEqual(1, PlayConsoleWatch.Errors);
            Assert.AreEqual("Firebase login failed", PlayConsoleWatch.First, "rich-text tags are console styling, not the message");
            PlayConsoleWatch.Reset();
        }
