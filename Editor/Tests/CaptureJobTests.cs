...
namespace ProjectNova.RecorderKit.Tests
{
    ...
    public class CaptureJobTests
    {
        ...
        public void JobGuard_RefusalReason_RefusesWrongGameAndUnknownKind_PassesTheRealMatch()
        {
            ...
            // a kind this kit does not handle (slice A2′ shipped `sync-nova`'s handler and slice
            // E.1 `probe`'s, so the stand-ins here are kinds that really have none)
            StringAssert.Contains("unsupported job kind",
                JobGuard.RefusalReason("rewind", "snl-scratch", "snl-scratch"));
            StringAssert.Contains("unsupported job kind",
        ...
        public void JobGuard_SelfTest_IsHandled_AndNeverRefusedOnIdentity()
        {
            ...
            Assert.IsFalse(CaptureJob.IsHandled("rewind"));
            // Slice D part 1: the export handler ships in THIS kit, so the kind is handled now. (The
            // API still withholds it from any kit below 0.6.0 — that gate is the sender's.)
            Assert.IsTrue(CaptureJob.IsHandled("export"));
            // Slice A2′: and so does sync-nova's.
            Assert.IsTrue(CaptureJob.IsHandled("sync-nova"));
            Assert.IsFalse(CaptureJob.IsHandled("teleport"), "an unknown kind is still refused whole");
        }

        // ---- slice A2′ — "Send to my editor" on the wire -------------------------------------

        [Test]
        public void JobGuard_SyncNova_IsNeverRefusedOnThisProjectsGameId()
        {
            // THIS JOB DELIVERS adapter.json. On a fresh project there is nothing to compare
            // against, and on a re-send the hosted gameId is the truth being written — so refusing
            // it here would refuse exactly the job that fixes a wrong one. The workspace binding is
            // what protects it (the server only hands the kind to a bound editor, and
            // WorkspaceGuard.Refusal checks it again), plus SyncNova.Refusal comparing the
            // DELIVERED adapter against the JOB — sender against sender (invariant 100).
            Assert.IsNull(JobGuard.RefusalReason("sync-nova", "snl-scratch", null));
            Assert.IsNull(JobGuard.RefusalReason("sync-nova", "snl-scratch", "unregistered"));
            Assert.IsNull(JobGuard.RefusalReason("sync-nova", "snl-scratch", "askie"));
            // the control: a CAPTURE on the same values still refuses
            Assert.IsNotNull(JobGuard.RefusalReason("capture", "snl-scratch", "askie"));
        }

        [Test]
        public void ParseClaim_ReadsTheTwoFilesASyncNovaCarries()
        {
            var job = CaptureJob.ParseClaim(
                @"{ ""job"": { ""runId"": ""r1"", ""workspaceId"": ""w"", ""kind"": ""sync-nova"", ""gameId"": ""g"" },
                    ""leaseUntil"": ""x"",
                    ""files"": { ""shots"": ""S"", ""adapter"": ""A"", ""shotsSha256"": ""aa"", ""adapterSha256"": ""bb"" } }",
                out var error, out var files);
            Assert.IsNull(error);
            Assert.AreEqual("sync-nova", job!.Kind);
            CollectionAssert.IsEmpty(job.Items, "this kind carries no items");
            Assert.IsNotNull(files);
            Assert.AreEqual("S", files!.Shots);
            Assert.AreEqual("A", files.Adapter);
            Assert.AreEqual("aa", files.ShotsSha256);
            Assert.AreEqual("bb", files.AdapterSha256);
        }

        [Test]
        public void ParseClaim_ASyncNovaWithNoFilesStillParsesSoItCanBeRefusedByName()
        {
            // The job must PARSE: a claim that fails to parse is dropped with a poll error, and the
            // run sits "running in your editor" until the lease lapses. It has to reach the handler
            // so the handler can report the reason through `done`.
            var job = CaptureJob.ParseClaim(
                @"{ ""job"": { ""runId"": ""r1"", ""workspaceId"": ""w"", ""kind"": ""sync-nova"" }, ""leaseUntil"": ""x"" }",
                out var error, out var files);
            Assert.IsNull(error);
        ...
        [Test]
        public void ParseClaim_AnIncompleteFilesObjectIsNoFilesAtAll()
        {
            foreach (var partial in new[]
                     {
                         @"{ ""shots"": ""S"", ""adapter"": ""A"", ""shotsSha256"": ""aa"" }",
                         @"{ ""shots"": ""S"", ""adapter"": """", ""shotsSha256"": ""aa"", ""adapterSha256"": ""bb"" }",
                         @"{ ""shots"": 3, ""adapter"": ""A"", ""shotsSha256"": ""aa"", ""adapterSha256"": ""bb"" }",
                         @"[]",
                     })
            {
                CaptureJob.ParseClaim(
                    @"{ ""job"": { ""runId"": ""r1"", ""kind"": ""sync-nova"" }, ""files"": " + partial + " }",
                    out _, out var files);
                Assert.IsNull(files, "a half-delivered pair must never be written: " + partial);
            }
        }

        [Test]
        public void ParseClaim_FilesOnAnyOtherKindAreIgnored()
        {
            CaptureJob.ParseClaim(
            ...
            // …and the two-argument overload still reads a plain claim, unchanged.
            var job = CaptureJob.ParseClaim(
                @"{ ""job"": { ""runId"": ""r9"", ""items"": [] }, ""leaseUntil"": ""x"" }", out var e);
            Assert.IsNull(e);
            Assert.AreEqual("r9", job!.RunId);
        }

        [Test]
        public void SyncNovaProgress_IsNotDoneUntilItsResultIsPosted()
        {
            // Same reload trap as the self-test: no items, so `Done` would be true the instant the
            // job started and the agent that came back would report a run it never ran.
            var job = new CaptureJob("run-sn", "ws1", Array.Empty<CaptureJobItem>(), 1800,
                "sync-nova", "g");
            var p = CaptureProgress.Start(job);
            Assert.IsTrue(p.ItemsDone);
            Assert.IsFalse(p.Done, "a sync-nova owes a result");
            p.ResultPosted = true;
            Assert.IsTrue(p.Done);
        }
        ...
        /// <summary>
        /// The thirteenth audit, M1 — AN ITEM THE CAPTURE AGENT FAILS HANDS THE EDITOR BACK. The unknown-shot path recorded
        /// the failure, saved the progress file and went straight on to the next item, which re-reads shots.json: a job of K
        /// items naming shots the file no longer holds ran K loads and K file writes inside one editor tick. The path goes
        /// through this seam now, which yields once after the save.
        /// </summary>
        [Test]
        public void FailItem_RecordsTheItemSavesTheProgressAndYieldsOnce()
        {
            var p = CaptureProgress.Start(CaptureJob.ParseList(ListJson, out _)[0]);
            var file = Path.Combine(_root, "capture-status.json");
            var yields = 0;
            foreach (var _ in NovaCaptureAgent.FailItem(p, file, "unknown shot in this project's shots.json"))
                if (++yields > 10) break;
            Assert.AreEqual(1, yields, "a failed item must hand the editor back once");
            Assert.AreEqual(1, p.Failed);
            Assert.AreEqual(1, p.NextIndex);
            var back = CaptureProgress.Load(file)!;
            Assert.AreEqual(1, back.Failed, "the failure is on disk before the yield");
            Assert.AreEqual(1, back.NextIndex);
        }
