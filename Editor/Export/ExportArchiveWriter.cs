...
namespace ProjectNova.RecorderKit
{
    ...
    public sealed class ExportArchiveWriter : IDisposable
    {
        ...
        /// <summary>Bytes written so far: every closed part plus the open one's current length.</summary>
        public long BytesWritten
        {
            get
            {
                long n = 0;
                foreach (var p in _parts) n += p.Bytes;
                try { if (_fs != null) n += _fs.Length; } catch (Exception) { }
                return n;
            }
        }
