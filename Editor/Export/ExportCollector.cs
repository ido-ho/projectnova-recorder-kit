    ...
    public static class ExportCollector
    {
        ...
        private sealed class Job
        {
            ...
            private IEnumerable<int> Addressables(JObject into)
            {
                ...
                if (haveGroupDir)
                {
                    ...
                    foreach (var f in files)
                    {
                        ...
                        foreach (var g in GroupGuids(f)) guids.Add(g);
            ...
            private void ConsiderDoc(string rel, string prefix, string kind)
            {
                ...
                if (length > _caps.DocFileMaxBytes) { Over(rel, length, kind, ExportFormat.ReasonOverDocFile); return; }
                ...
                _docItems.Add(new Item
                {
                    ...
                    Source = abs,
                    OverKind = kind,
                    MaxBytes = _caps.DocFileMaxBytes,
                    OverReason = ExportFormat.ReasonOverDocFile,
                });
            }
            ...
            private void ConsiderArt(Row r, string kind)
            {
                ...
                if (length > _caps.ArtFileMaxBytes) { Over(r.Path, length, kind, ExportFormat.ReasonOverArtFile); return; }
                ...
                _artItems.Add(new Item
                {
                    ...
                    Source = abs,
                    OverKind = kind,
                    MaxBytes = _caps.ArtFileMaxBytes,
                    OverReason = ExportFormat.ReasonOverArtFile,
                });
            }
            ...
            {
                if (a.InBuild != b.InBuild) return a.InBuild ? -1 : 1;
            ...
            private IEnumerable<int> WriteArchive()
            {
                ...
                var filesWritten = 0;
                var filesTotal = _docItems.Count + _artItems.Count;
